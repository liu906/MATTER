...
draw_boxplot <- function(data,xlab_string='',title='',prefix){
  if(xlab_string=='IND-JLMIV+R-1Y_change59'){
    xlab_string = 'IND-JLMIV+R-1Y'
  }
  if(xlab_string=='ALLJURECZKO'){
    xlab_string = 'JURECZKO'
  }
  ...
  p = data %>%
    ...
    theme(
      ...
      axis.text.x = element_text(angle = x_label_angle,vjust = 0.3,hjust=0.9,size = font_size),
...
# level='one' mean one-level rank. directly rank by the performance indicator
batch_plot(prefix = 'stateOfArt',
           thresholds = c('-1','0.2','20'),
           indicators = c('mcc','roi_tp','ifap2'),
           flag='all',
